...
module.exports = server => {
    server.get('/keywords', async (req, res, next) => {
        //GET all keywords
        try{
    ...
    server.post('/keywords', async (req, res, next) => {
        ...
        const { messages, queueItemId, device } = req.body;
  ...
  server.put('/keywords/:id', async (req, res, next) => {
      ...
      try {
        const keyword = await Keywords.findOneAndUpdate({_id: req.params.id}, req.body);
        res.send(200);
        next();
      } catch (err) {
        return next(
    ...
    server.del('/keywords/:id', async (req, res, next) => {
        try{
            const keyword = await Keywords.findOneAndRemove({_id: req.params.id});
            res.send(204);
            next();
        }   catch(err){
            return next(
